...
* `--log-config <path>` - Logging configuration file. **Options:** *`dictConfig()` formats: .json, .yaml, .toml*. Any other format will be processed with `fileConfig()`. Set the `formatters.default.use_colors` and `formatters.access.use_colors` values to override the auto-detected behavior.
    * If you wish to use a YAML file or TOML file (with Python 3.10 and lower) for your logging config, you will need to include PyYAML or tomli as a dependency for your project or install uvicorn with the `[standard]` optional extras.
